...
export default function Pick3Tool() {
  ...
  const draws  = useMemo(() => rawData.length ? parseDraws(rawData,"pick3",dt) : [], [rawData,dt]);
  const eng    = useMemo(() => draws.length ? runEngine(draws,"pick3",win) : null, [draws,win]);
  ...
  const wheelCombos = useMemo(() => {
    ...
    tops[0].forEach(a => tops[1].forEach(b => tops[2].forEach(c => {
      const k = a+b+c; if (!seen.has(k)) { seen.add(k); out.push([a,b,c]); }
    })));
    return out;
  }, [ranked]);
  ...
  return (
    <div style={{padding:"40px 32px", maxWidth:960, margin:"0 auto"}}>
      <div className="fade-up" style={{marginBottom:32}}>
        <h1 style={{fontFamily:"'DM Serif Display',Georgia,serif", fontSize:36, color:T.text, margin:"0 0 6px", fontWeight:400}}>Pick 3 Analysis Engine</h1>
        <p style={{fontSize:14, color:T.textM, margin:0}}>NY Daily Numbers · 9 angles · live data · transparent scoring</p>
      </div>
